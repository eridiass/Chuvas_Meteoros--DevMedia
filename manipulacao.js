...
];//Declaração de conjunto de objetos

let titulo = `=== chuvas de meteoros - hemisfério sul ===`;
console.log(titulo.toUpperCase());
console.log("\n");//Dados para cabeçalho

for ( const chuva of chuvas_meteoros ){//Iteração de dados

    let nome_chuva = chuva.nome;
    let pico_chuva = chuva.pico;
    let velocidade_chuva = chuva.velocidade;//Isolando dados

    let pico_mes_dia = pico_chuva.split(" ");//Separação de dados

    let pico_dia = pico_mes_dia[1];
    let pico_mes = pico_mes_dia[0];
    let pico_mes_nome = "";//Preparação de dados

    switch (pico_mes) {//Escolha, caso..
    ...
    let velocidade_formatada = velocidade_chuva.replace(`km/s`, `quilômetros por segundo`);//Alterações para apresentação
